...
public class InntektSyntPakkenServiceTest {
    ...
    public void shouldGenerereInntektsmeldinger() {
        ...
        when(inntektSyntConsumer.startSyntetisering(syntetiserInntektsmeldingRequest)).thenReturn(expectedFeiledeInntektsmeldinger);

        Map<String, List<Object>> response = inntektSyntPakkenService.genererInntektsmeldinger(syntetiserInntektsmeldingRequest);

        assertThat(response.get(fnr1), IsIterableContainingInOrder.contains(inntektsmelding));
        verify(inntektSyntConsumer).startSyntetisering(syntetiserInntektsmeldingRequest);
    }
}