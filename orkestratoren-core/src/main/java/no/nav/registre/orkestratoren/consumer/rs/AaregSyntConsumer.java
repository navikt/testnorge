...
public class AaregSyntConsumer {
    ...
    public AaregSyntConsumer(@Value("${testnorge-aareg.rest-api.url}") String aaregServerUrl) {
        this.url = new UriTemplate(aaregServerUrl + "/v1/syntetisering/generer");
    }

    @Timed(value = "orkestratoren.resource.latency", extraTags = { "operation", "aareg" })
    public ResponseEntity startSyntetisering(SyntetiserAaregRequest syntetiserAaregRequest) {
        RequestEntity postRequest = RequestEntity.post(url.expand()).body(syntetiserAaregRequest);
