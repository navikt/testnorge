...
public class FnrGenerator {
    ...
    public LinkedHashSet<String> genererIdenter(LocalDate fom, LocalDate tom) {
        if (fom.isAfter(tom)) {
            throw new IllegalArgumentException(String.format("Dato fom %s, må være før dato tom %s", fom.toString(), tom.toString()));
        }
        tom = tom.plusDays(1);
