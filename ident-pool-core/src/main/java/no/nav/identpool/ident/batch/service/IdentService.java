...
import no.nav.identpool.ident.batch.mq.QueueContext;
...
import no.nav.identpool.ident.batch.tps.generator.FnrGenerator;
import no.nav.identpool.ident.batch.tps.xml.service.Personopplysning;
...
class IdentService {
    ...
    void generateForDates(LocalDate ...dates) throws JMSException {
        ...
        System.out.println(Arrays.stream(this.identer[0]).filter(Objects::nonNull).map(this::createDefaultIdent).collect(Collectors.toList()).size());
        System.out.println(Arrays.stream(this.identer[1]).filter(Objects::nonNull).map(this::createDefaultIdent).collect(Collectors.toList()).size());
        System.out.println(Arrays.stream(this.identer[2]).filter(Objects::nonNull).map(this::createDefaultIdent).collect(Collectors.toList()).size());
    }
