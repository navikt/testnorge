...
describe("EksisterendeIdent.js", () => {
  ...
  const renderAttributtModus = shallow(
    <EksisterendeIdent
      eksisterendeIdentListe={["25043679458", "31049178459"]}
      ugyldigIdentListe={["12345678921", "45678912365"]}
    />
  );
  ...
  const renderAttributtModusIngenGyldige = shallow(
    <EksisterendeIdent
      ...
      ugyldigIdentListe={["25043679458", "31049178459"]}
    />
  );

  it("should render skrivemodus", () => {
    expect(
      renderSkrivemodus.find(".eksisterende-fnr-inntasting").exists()
    ).toBe(true);
  });
  ...
  it("should render attributtModus med bare gyldige fnr", () => {
    expect(
      renderAttributtModusBareGyldige.find(".ugyldig-container").exists()
    ).toBe(false);
  });

  it("should render attributtModus uten gyldige fnr", () => {
    expect(
      renderAttributtModusIngenGyldige.find(".ingen-gyldige-identer").exists()
    ).toBe(true);
    expect(renderAttributtModusIngenGyldige.find(".ugyldig").exists()).toBe(
      true
    );
  });
});
