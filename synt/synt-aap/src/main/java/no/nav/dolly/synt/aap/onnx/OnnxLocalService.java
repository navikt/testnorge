...
import no.nav.dolly.synt.aap.bucket.LocalModels;
...
@Profile("local")
@Slf4j
public class OnnxLocalService implements OnnxService {
    ...
    public List<AapVedtakDto> generateAap(List<VedtakRequestDto> requests, boolean brukInnsendtTilDato) {
        return generator.generateVedtak(AapModelType.AAP, requests, brukInnsendtTilDato)
                .stream()
    ...
    public List<Vedtak115Dto> generate115(List<VedtakRequestDto> requests) {
        return generator.generateVedtak(AapModelType.AAP_115, requests, true)
                .stream()
    ...
    public List<AatforAaunguforFriMkVedtakDto> generateFriMk(List<VedtakRequestDto> requests) {
        return generator.generateVedtak(AapModelType.FRI_MK, requests, true)
                .stream()
    ...
    public List<AatforAaunguforFriMkVedtakDto> generateAaungufor(List<VedtakRequestDto> requests) {
        return generator.generateVedtak(AapModelType.AAUNGUFOR, requests, true)
                .stream()
                .map(AapVedtakMapper::toAatforAaunguforFriMkVedtak)
                .toList();
    }
    ...
    public List<AatforAaunguforFriMkVedtakDto> generateAatfor(List<VedtakRequestDto> requests) {
        return generator.generateVedtak(AapModelType.AATFOR, requests, true)
                .stream()
                .map(AapVedtakMapper::toAatforAaunguforFriMkVedtak)
                .toList();
    }
