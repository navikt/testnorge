...
public class EksisterendeIdenterSkdServiceTest {
    ...
    public void shouldFindLevendeNordmannAndUpdateBrukteIdenter() {
        ...
        verify(hodejegerenConsumer, times(2)).getStatusQuoFraEndringskode(any(), any(), any());
    ...
    public void shouldHandleIdenterWithNullValuesInStatusQuoFields() {
        ...
        verify(hodejegerenConsumer, times(3)).getStatusQuoFraEndringskode(any(), any(), any());
        assertEquals(1, meldinger.size());
    ...
    public void shouldFindUgiftMyndigPersonAndCreateVigselsmelding() {
        ...
        verify(hodejegerenConsumer, times(4)).getStatusQuoFraEndringskode(any(), any(), any());
    ...
    private List<String> opprettEkteparMedKorruptDataMock() {
        ...
        when(hodejegerenConsumer.getStatusQuoFraEndringskode(any(), eq(environment), eq(fnr1))).thenReturn(statusQuo);
        ...
        when(hodejegerenConsumer.getStatusQuoFraEndringskode(any(), eq(environment), eq(fnr2))).thenReturn(statusQuo);

        // Et fungerende ektepar
        statusQuo = new HashMap<>();
        statusQuo.put(SIVILSTAND, GIFT.getSivilstandKodeSKD());
        statusQuo.put(FNR_RELASJON, fnr3);
        when(hodejegerenConsumer.getStatusQuoFraEndringskode(any(), eq(environment), eq(fnr4))).thenReturn(statusQuo);
        statusQuo = new HashMap<>();
        statusQuo.put(SIVILSTAND, GIFT.getSivilstandKodeSKD());
        statusQuo.put(FNR_RELASJON, fnr4);
        when(hodejegerenConsumer.getStatusQuoFraEndringskode(any(), eq(environment), eq(fnr3))).thenReturn(statusQuo);
    ...
    /**
     * Testscenario: HVIS metoden {@link HodejegerenConsumer#getStatusQuoFraEndringskode} kaster
    ...
    private void opprettLevendeNordmennMock() {
        ...
        when(hodejegerenConsumer.getStatusQuoFraEndringskode(any(), any(), eq(fnr1))).thenReturn(statusQuo);

        statusQuo = new HashMap<>();
        statusQuo.put(DATO_DO, "");
        statusQuo.put(STATSBORGER, "NORGE");
        when(hodejegerenConsumer.getStatusQuoFraEndringskode(any(), any(), eq(fnr2))).thenReturn(statusQuo);
    }
    ...
    private void opprettIdenterMedManglendeFeltMock() {
        ...
        statusQuo = new HashMap<>();
        statusQuo.put(STATSBORGER, "NORGE");
        when(hodejegerenConsumer.getStatusQuoFraEndringskode(any(), any(), eq(fnr3))).thenReturn(statusQuo);
    }
    ...
    private void opprettMultipleUgifteIdenterMock() {
        ...
        Map<String, String> statusQuo = new HashMap<>();
        statusQuo.put(SIVILSTAND, KoderForSivilstand.UGIFT.getSivilstandKodeSKD());
        when(hodejegerenConsumer.getStatusQuoFraEndringskode(any(), eq(environment), eq(fnrUmyndig))).thenReturn(statusQuo);
    ...
    private void opprettMultipleGifteIdenterWithStatusQuoErrorMock() {
        ...
        when(hodejegerenConsumer.getStatusQuoFraEndringskode(any(), eq(environment), eq(fnr2))).thenThrow(new ManglendeInfoITpsException());
