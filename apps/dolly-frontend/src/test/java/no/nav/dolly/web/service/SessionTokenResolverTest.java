...
class SessionTokenResolverTest {
    ...
    void shouldReturnTokenWhenManagerReturnsAuthorizedClient() {
        ...
        StepVerifier.create(
                        sessionTokenResolver.getToken(exchange)
    ...
    void shouldHandleTokenWithoutRefreshToken() {
        ...
        StepVerifier.create(
                ...
                .assertNext(token -> {
                    assertThat(token.getAccessTokenValue()).isEqualTo("access-token-only");
    ...
    @Test
    void shouldDelegateToManagerWithCorrectRegistrationId() {
        var accessToken = new OAuth2AccessToken(
                OAuth2AccessToken.TokenType.BEARER,
                "token",
                Instant.now(),
