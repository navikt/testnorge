...
export const SlettTeam = ({ team, closeModal, mutate }) => {
	const [isLoading, setIsLoading] = useState(false)
	const [error, setError] = useState('')

	const handleSlettTeam = () => {
		setIsLoading(true)
		setError('')
		DollyApi.slettTeam(team.id)
			.then((response) => {
	...
	return (
		<div className="slettModal">
			...
			<div className="slettModal slettModal-content">
				<h1>Slett team</h1>
				<h4>{`Er du sikker på at du vil slette team ${team.navn}?`}</h4>
			...
			<div className="dollymodal_buttons">
				...
				<NavButton variant={'primary'} onClick={handleSlettTeam} loading={isLoading}>
					Ja, jeg er sikker
				</NavButton>
			</div>
		</div>
