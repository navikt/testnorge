...
const BESTILLING_FAGSYSTEM_MAP: Record<string, { id: string; navn: string }> = {
	...
	sigrunstubSummertSkattegrunnlag: {
		id: 'SIGRUN_SUMMERT',
		navn: 'Summert skattegrunnlag (Sigrunstub)',
	},
	...
	'pensjonforvalter.alderspensjonNyUtaksgrad': {
		id: 'PEN_AP_NY_UTTAKSGRAD',
		navn: 'Ny uttaksgrad (AP)',
	},
...
export const calculateProgress = ({
	...
	if (
		!erOrganisasjon &&
		antallIdenter === 1 &&
		(statusList.length > 0 || (expectedTotal && expectedTotal > 0))
	) {
		...
		text = `${completedCount} av ${total} steg fullført`
	} else {
