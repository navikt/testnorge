...
test('Dolly Bestillingsdetaljer testing', async ({ page, context }) => {
	...
	await expect(
		page.getByTestId(TestComponentSelectors.BUTTON_GJENOPPRETT_GRUPPE).last(),
	).toBeDisabled()
	await page.locator(':nth-child(2) > .dot-body-row > .dot-body-row-columns').click()
	await page.locator(':nth-child(3) > .dot-body-row > .dot-body-row-columns').click()
	await page.getByTestId(TestComponentSelectors.BUTTON_GJENOPPRETT_GRUPPE).last().click()
	await page
	...
	await page.getByLabel('Navn på mal').fill('Testmal')
	await page.getByRole('button', { name: 'Lagre mal' }).click()
})
