package no.nav.registre.sdforvalter.domain;

import lombok.experimental.UtilityClass;

@UtilityClass
public final class CommonKeysAndUtils {

    public static final String HEADER_NAV_CONSUMER_ID = "Nav-Consumer-Id";
    public static final String HEADER_NAV_CALL_ID = "Nav-Call-Id";
    public static final String CONSUMER = "testnorge-statisk-data-forvalter";

    public static final String HEADER_NAV_ARBEIDSFORHOLD = "Nav-Arbeidsforhold-Kildereferanse";

}
