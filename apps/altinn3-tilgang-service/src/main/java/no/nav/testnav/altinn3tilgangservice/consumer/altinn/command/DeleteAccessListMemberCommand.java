...
public class DeleteAccessListMemberCommand implements Callable<Mono<DeleteStatus>> {
    ...
    private final OrganisasjonDataDTO organisasjonDataDTO;
    ...
    public Mono<DeleteStatus> call() {
        ...
        return webClient
                ...
                .bodyValue(organisasjonDataDTO)
                .retrieve()
                .toBodilessEntity()
                .map(resultat -> DeleteStatus.builder()
                        .status(HttpStatus.valueOf(resultat.getStatusCode().value()))
                        .build())
                .doOnSuccess(value -> log.info("Organiasjon tilgang {} slettet.", organisasjonDataDTO.getData()))
                .doOnError(
