...
public class TokenController {
    ...
    @PostMapping
    public ResponseEntity<Mono<TokenDTO>> getToken(@RequestBody ApplicationDTO applicationDTO, ServerWebExchange exchange) {
        var token = tokenService.getToken(new Application(applicationDTO), exchange).map(TokenDTO::new);
        return ResponseEntity.ok(token);
    }
