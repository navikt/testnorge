package no.nav.testnav.apps.statusfrontend.fagsystem.fullmakt;
...
@Service
@ConditionalOnProperty(prefix = "functional-test.fullmakt", name = "technical-only", havingValue = "true")
public class FullmaktTechnicalStatus implements TechnicalStatusDefinition {

    private static final TechnicalStatusDescriptor DESCRIPTOR = new TechnicalStatusDescriptor(
            new SystemId("fullmakt"),
            new DisplayName("Fullmakt"),
            Set.of(FunctionalTestEnvironment.GLOBAL));

    private final SecondBatchTechnicalStatusClient client;

    public FullmaktTechnicalStatus(SecondBatchTechnicalStatusClient client) {
        this.client = client;
    }
    ...
    public Mono<Void> check(FunctionalTestContext context) {
        return client.checkFullmakt(context.runId());
    }
}
