...
import static no.nav.testnav.levendearbeidsforholdscheduler.utils.Utils.beregnEkstraDelayNaa;
import static no.nav.testnav.levendearbeidsforholdscheduler.utils.Utils.hentKalender;
...
public class JobbScheduler {
    ...
    public Optional<String> hentTidspunktNesteKjoring() {
        ...
        if (nonNull(scheduledFuture)) {
            ...
            var delayIMillisekunder = scheduledFuture.getDelay(TimeUnit.MILLISECONDS);
            var nesteKjoringMillisekunder = beregnNesteKjoring(delayIMillisekunder);

            var datoForNesteKjoring = new Date(nesteKjoringMillisekunder);
    ...
    public boolean sjekkOmGyldigTidsromNaa(int startKlokka, int startDag, int sluttKlokka, int sluttDag) {
        ...
        var kalender = hentKalender();
        var klokkeslett = kalender.get(Calendar.HOUR_OF_DAY); //06:00 = 6, 13:00 = 13, 23:00 = 23
        var dag = ((kalender.get(Calendar.DAY_OF_WEEK) + 6) % 7); //Mandag = 1, Tirsag = 2 ... Søndag = 7
    ...
    public long beregnNesteKjoring(long schedulerDelay) {
        ...
        var nesteKjoringMillisekunder = System.currentTimeMillis() + schedulerDelay;
    ...
    private class AnsettelseJobb implements Runnable {
        ...
        public void run() {
            ...
            if (sjekkOmGyldigTidsromNaa(START_KLOKKESLETT, START_DAG, SLUTT_KLOKKESLETT, SLUTT_DAG)) {
                ...
                ansettelseConsumer.opprettAnsettelser()
                        .subscribe(resultat -> log.info("Startet oppretting av ansettelser, status: %s".formatted(resultat)));
            }
        }
    }
}
