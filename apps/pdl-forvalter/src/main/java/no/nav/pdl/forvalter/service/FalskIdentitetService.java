...
public class FalskIdentitetService implements Validation<FalskIdentitetDTO> {
    ...
    public Mono<DbPerson> convert(DbPerson dbPerson) {
        ...
        return Flux.fromIterable(dbPerson.getPerson().getFalskIdentitet())
    ...
    private Mono<Void> opprettNyIdentitet(FalskIdentitetDTO identitet, PersonDTO person) {
        ...
        return createPersonService.execute(identitet.getNyFalskIdentitetPerson())
                ...
                .flatMap(rettIdenitet -> relasjonService.setRelasjoner(person.getIdent(),
                        RelasjonType.RIKTIG_IDENTITET, identitet.getRettIdentitetVedIdentifikasjonsnummer(),
                        RelasjonType.FALSK_IDENTITET));
    }
