...
public class ArtifactGjeldendeService {
    ...
    public void setGjeldendeForRelasjon(String ident) {
        ...
        setGjeldendeForIdent(ident);

        personRepository.findByIdent(ident)
                .ifPresent(hovedperson -> hovedperson.getRelasjoner().stream()
                        .map(DbRelasjon::getRelatertPerson)
                        .map(DbPerson::getIdent)
                        .forEach(this::setGjeldendeForIdent));
    }

    private void setGjeldendeForIdent(String ident) {
