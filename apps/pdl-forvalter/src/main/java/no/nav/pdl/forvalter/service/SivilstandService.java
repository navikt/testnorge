...
public class SivilstandService implements Validation<SivilstandDTO> {
    ...
    private static boolean isMyndig(LocalDateTime foedselsdato) {

        return foedselsdato.toLocalDate().isBefore(LocalDate.now().minusYears(18)) ||
                foedselsdato.toLocalDate().isEqual(LocalDate.now().minusYears(18));
    }
    ...
    private void handle(SivilstandDTO sivilstand, PersonDTO hovedperson) {
        ...
        if (isNull(sivilstand.getType()) && isMyndig(hovedperson.getFoedsel().stream().findFirst()
                .orElse(FoedselDTO.builder().foedselsdato(DatoFraIdentUtility.getDato(hovedperson.getIdent()).atStartOfDay()).build())
                .getFoedselsdato())) {

            sivilstand.setType(UGIFT);

        } else if (sivilstand.getType() == GIFT ||
                sivilstand.getType() == REGISTRERT_PARTNER) {
    ...
    protected void enforceIntegrity(List<SivilstandDTO> sivilstand) {
        ...
        var iterator = sivilstand.iterator();
        while (iterator.hasNext()) {
            if (isNull(iterator.next().getType())) {
                iterator.remove();
            }
        }
