...
class GetKollegaerIOrganisasjonCommandTest {
    ...
    @Test
    void shouldGetOrganizationUsersWithBearerToken() throws Exception {
        ...
        StepVerifier.create(new GetKollegaerIOrganisasjonCommand(
                        webClient,
                        "hashed-id",
                        "exchanged-token").call())
                .assertNext(response -> assertThat(response.getBrukere())
    ...
    @Test
    void shouldMapDependencyFailureToServiceUnavailable() throws Exception {
        wireMockServer.stubFor(get(urlPathEqualTo("/api/v1/tilgang"))
                .withQueryParam("brukerId", equalTo("hashed-id"))
