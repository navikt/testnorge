...
public class Inntektsmottaker {
    ...
    public Inntektsmottaker(
            ...
            no.nav.registre.testnorge.xsd.arbeidsforhold.v2_1.Inntektsmottaker inntektsmottaker
    ) {
    ...
    private static int getAntallUtenArbeidsforholdMatch(
            List<no.nav.registre.testnorge.xsd.arbeidsforhold.v2_1.Arbeidsforhold> arbeidsforholdList,
            List<no.nav.registre.testnorge.xsd.arbeidsforhold.v2_1.Inntekt> inntektList
    ) {
        if (inntektList == null) {
            return 0;
        }
        var ids = inntektList.stream().map(no.nav.registre.testnorge.xsd.arbeidsforhold.v2_1.Inntekt::getArbeidsforholdId).collect(Collectors.toList());
        return (int) ids.stream().filter(id -> !contains(id, arbeidsforholdList)).count();
    }

    private static boolean contains(String arbeidsforholdId, List<no.nav.registre.testnorge.xsd.arbeidsforhold.v2_1.Arbeidsforhold> arbeidsforholdList) {
    ...
    private static int getAntallInntekter(String arbeidsforholdId, List<no.nav.registre.testnorge.xsd.arbeidsforhold.v2_1.Inntekt> inntektList) {
        if (inntektList == null) {
            return 0;
        }
    ...
    private static String getTypeArbeidsforholdFraIntekt(
            no.nav.registre.testnorge.xsd.arbeidsforhold.v2_1.Inntekt inntekt,
            List<no.nav.registre.testnorge.xsd.arbeidsforhold.v2_1.Arbeidsforhold> arbeidsforholdList
    ) {
