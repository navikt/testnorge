...
public class Testgruppe implements Serializable {
    ...
    public boolean equals(Object o) {
        ...
        return new EqualsBuilder().append(id, that.id).append(versjon, that.versjon).append(navn, that.navn).append(hensikt, that.hensikt).append(opprettetAv, that.opprettetAv).append(sistEndretAv, that.sistEndretAv).append(datoEndret, that.datoEndret).append(testidenter, that.testidenter).append(favorisertAv, that.favorisertAv).append(bestillinger, that.bestillinger).append(erLaast, that.erLaast).append(laastBeskrivelse, that.laastBeskrivelse).append(tags, that.tags).isEquals();
    }

    @Override
    public int hashCode() {
        return new HashCodeBuilder(17, 37).append(id).append(versjon).append(navn).append(hensikt).append(opprettetAv).append(sistEndretAv).append(datoEndret).append(testidenter).append(favorisertAv).append(bestillinger).append(erLaast).append(laastBeskrivelse).append(tags).toHashCode();
    }
