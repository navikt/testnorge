...
import no.nav.dolly.bestilling.bistandsbehov.command.SlettBistandsvedtakCommand;
    ...
    public Flux<ResponseStatusDTO> slettBistandsvedtak(List<String> identer) {
        ...
        return tokenService.exchange(serverProperties)
                .flatMapMany(token -> Flux.fromIterable(identer)
                        .flatMap(ident -> new SlettBistandsvedtakCommand(webClient, ident, token.getTokenValue()).call()));
    }
}
